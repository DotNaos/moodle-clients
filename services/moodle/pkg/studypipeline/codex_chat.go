...
func RunCodexChat(ctx context.Context, input CodexChatInput) (contract.CodexRunResponse, error) {
	...
	// Streaming chat must use the Codex app-server protocol. In local testing
	// with Codex CLI 0.130.0, `codex exec --json` emitted the final
	// agent_message only after generation completed, while app-server emitted
	// live item/agentMessage/delta notifications.
	if input.Emit != nil && len(input.OutputSchema) == 0 {
		return runCodexChatAppServer(ctx, input, model, reasoningEffort, image)
	}
...
func normalizeCodexActions(actions []contract.CodexUIAction) []contract.CodexUIAction {
	...
	for _, action := range actions {
		...
		case "open_resource", "read_material_text":
