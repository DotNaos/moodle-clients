...
func (s *Store) listActiveStudyPipelineSelections(ctx context.Context, sourceID string) ([]ActiveRunSelectionRecord, error) {
	rows, err := s.db.QueryContext(ctx, `
		select selections.source_id, selections.resource_id, selections.stage, selections.active_run_id::text,
			coalesce(selections.selected_by::text, ''), selections.selected_at, selections.reason
		from active_run_selections selections
		join study_pipeline_runs runs on runs.id = selections.active_run_id
		where selections.source_id = $1
		order by selections.selected_at desc
	`, sourceID)
	if err != nil {
		return nil, err
	}
...
func normalizeStudyPipelineRecordInput(input *StudyPipelineRecordInput) {
	...
	input.Stage = normalizeStudyPipelineStage(input.Stage)
