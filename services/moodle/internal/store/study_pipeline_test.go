...
func TestNormalizeStudyPipelineStageMapsUiAliasesToDatabaseStages(t *testing.T) {
	...
	for input, want := range cases {
		if got := normalizeStudyPipelineStage(input); got != want {
			t.Fatalf("normalizeStudyPipelineStage(%q) = %q, want %q", input, got, want)
		}
	}
}
