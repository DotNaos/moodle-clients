...
func TestStudyPipelinePlanRouteNormalizesCodexStageAlias(t *testing.T) {
	t.Setenv(studypipeline.EnvArtifactRoot, t.TempDir())
	router, err := NewRouter(ServerOptions{
		ClientProvider: func() (Client, error) {
			return stubClient{
	...
	req := httptest.NewRequest(
		http.MethodPost,
		"/api/courses/22584/study-pipeline/plan",
		strings.NewReader(`{"mode":"single","startStage":"codex","model":"gpt-test"}`),
	)
	router.ServeHTTP(rec, req)
	...
	if len(payload.Steps) != 1 || payload.Steps[0].Stage != "curated" {
		t.Fatalf("expected codex alias to plan canonical curated stage, got %#v", payload.Steps)
	}
	if payload.Response != nil && payload.Response.Engine != "codex" {
		t.Fatalf("expected curated response to use codex engine, got %#v", payload.Response)
	}
}
...
func TestCoursesHandler(t *testing.T) {
	...
	if got[0].CategoryID != 42 {
		t.Fatalf("course normalization was not preserved: %#v", got[0])
	}
	if !strings.HasPrefix(got[0].HeroImage, "/api/course-images/1?v=") {
		t.Fatalf("expected localized course image URL, got %q", got[0].HeroImage)
	}
}

func TestCourseImageHandlerServesDataSVG(t *testing.T) {
	t.Setenv("DATABASE_URL", "")
	...
	router, err := NewRouter(ServerOptions{
		ClientProvider: func() (Client, error) {
			return stubClient{
				courses: []moodle.Course{
					{ID: 1, Fullname: "Course A", HeroImage: source},
				},
			}, nil
		},
	})
	...
	rec := httptest.NewRecorder()
	version := courseimages.SourceHash(source)[:16]
	req := httptest.NewRequest(http.MethodGet, "/api/course-images/1?v="+url.QueryEscape(version), nil)
	router.ServeHTTP(rec, req)
