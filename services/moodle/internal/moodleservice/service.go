...
func (s Service) MaterialText(courseID string, resourceID string) (FetchDocument, error) {
	...
	download, err := s.downloadMaterialResource(courseID, resource)
	if err != nil {
		return FetchDocument{}, err
	}
	text, textCached, err := s.materialTextFromDownload(resource, download)
	if err != nil {
		return FetchDocument{}, err
	}
	...
	return FetchDocument{
		...
		Metadata: map[string]string{
			...
			"textCached":  strconv.FormatBool(textCached),
		},
	}, nil
}
