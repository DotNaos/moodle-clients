...
func TestMobileClientFetchCoursesUsesOverviewSVGWhenCourseImageIsEmpty(t *testing.T) {
	...
	if courses[0].HeroImage != svgDataURI {
		t.Fatalf("expected SVG overview image %q, got %q", svgDataURI, courses[0].HeroImage)
	}
}
...
func TestMobileClientFetchCoursesUsesTimelineImageWhenUserCourseImageIsEmpty(t *testing.T) {
	...
	if courses[0].HeroImage != svgDataURI {
		t.Fatalf("expected SVG timeline image %q, got %q", svgDataURI, courses[0].HeroImage)
	}
}

func TestMobileClientFetchCoursesPrefersTimelineSVGDataURIOverGeneratedCourseImage(t *testing.T) {
	const svgDataURI = "data:image/svg+xml;base64,PHN2Zy8+"
	...
	if courses[0].HeroImage != svgDataURI {
		t.Fatalf("expected timeline SVG data URI %q, got %q", svgDataURI, courses[0].HeroImage)
	}
}
...
func TestMobileClientFetchCoursesKeepsInlineSVGTimelineImage(t *testing.T) {
	const svgDataURI = "data:image/svg+xml;base64,PHN2Zy8+"
	const detailImage = "https://moodle.fhgr.ch/pluginfile.php/1267822/course/overviewfiles/banner.jpg"
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if err := r.ParseForm(); err != nil {
...
func TestMobileClientFetchCoursesUsesCourseDetailImageWhenListsAreEmpty(t *testing.T) {
	...
	if courses[0].HeroImage != svgDataURI {
		t.Fatalf("expected SVG course detail image %q, got %q", svgDataURI, courses[0].HeroImage)
	}
}
