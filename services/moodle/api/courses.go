...
const (
	webexRecordingCacheMaxAge = 24 * time.Hour
)
...
func Courses(w http.ResponseWriter, r *http.Request) {
	if r.URL.Query().Get("route") == "course-image" {
		handleCourseImageAsset(w, r)
		return
	}
	...
	courses = courseimages.LocalizePaths(courses)
	svc.WriteJSON(w, http.StatusOK, contract.CoursesResponse{Courses: courses})
}

func handleCourseImageAsset(w http.ResponseWriter, r *http.Request) {
	if !svc.AllowMethods(w, r, http.MethodGet) {
		return
	}
	cfg := svc.LoadServerEnv()
	...
	if asset, err := st.CourseImageAsset(r.Context(), user.ID, courseID); err == nil {
		if version == "" || strings.HasPrefix(asset.SourceHash, version) {
			writeCourseImageAsset(w, r, asset)
			return
		}
	...
	source, sourceHash, err := courseImageSourceForCourse(service, courseID)
	if err != nil {
		svc.WriteError(w, err)
		return
	}
	if version != "" && !strings.HasPrefix(sourceHash, version) {
		svc.WriteError(w, svc.ErrNotFound)
		return
	}
	data, contentType, err := courseimages.Read(r.Context(), service.Client, source)
	if err != nil {
		svc.WriteError(w, err)
		return
	}
	if len(data) > courseimages.MaxBytes {
		svc.WriteError(w, fmt.Errorf("course image exceeds %d bytes", courseimages.MaxBytes))
		return
	}
	asset := svc.CourseImageAsset{
		UserID:      user.ID,
		CourseID:    courseID,
	...
	if err := st.UpsertCourseImageAsset(r.Context(), svc.UpsertCourseImageAssetInput{
		UserID:      asset.UserID,
...
func writeCourseImageAsset(w http.ResponseWriter, r *http.Request, asset svc.CourseImageAsset) {
	etag := `"` + asset.SourceHash + `"`
...
func courseImageSourceForCourse(service svc.Service, courseID string) (string, string, error) {
	courses, err := service.ListCourses()
	if err != nil {
		return "", "", err
	}
